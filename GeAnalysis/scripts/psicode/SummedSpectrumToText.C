...
void SummedSpectrumToText()
{
    ...
    //for (Int_t fileID = 0; fileID < 1; fileID++)
    {
        ...
        //cout << timeBinMin << "   " << timeBinMax << endl;

        // hMergedSpectrum[0] is with cuts for prompt peaks
        hMergedSpectrum.push_back((TH1F *)(hGermaniumEnergyVsTimeFine1->ProjectionZ("proj01", timeBinMin, timeBinMax, 1, 8)));   // MB14C for now has bad timing
...
#define MAKE_PHADATA 1
        ...
        phaDataName.ReplaceAll(".root", "_pha.dat");
        ...
        Int_t nbin = hMergedSpectrum[0]->GetNbinsX();
        for (Int_t i = 1; i < nbin + 1; i++)
        {
            Double_t binCenter = hMergedSpectrum[0]->GetBinCenter(i);
            Double_t binContent = hMergedSpectrum[0]->GetBinContent(i);
...
void ReadRootFileList()
{
    std::ifstream runListFile("run_list.txt");
    if (!runListFile)
    {
        return 1;
    }
